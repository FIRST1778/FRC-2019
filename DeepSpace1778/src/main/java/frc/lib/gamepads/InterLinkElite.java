package frc.lib.gamepads;

/**
 * Output mapping constants for the InterLink Elite controller.
 *
 * @author FRC 1778 Chill Out
 */
public class InterLinkElite {

  public static final int LEFT_SWITCH = 1;
  public static final int LEFT_SHOULDER_SWITCH = 5;
  public static final int RIGHT_SHOULDER_SWITCH = 2;
  public static final int RESET_BUTTON = 3;

  public static final int AXIS_LEFT_X = 4;
  public static final int AXIS_LEFT_Y = 2;
  public static final int AXIS_RIGHT_X = 0;
  public static final int AXIS_RIGHT_Y = 1;
  public static final int AXIS_TUNER = 3;
}
