...
public class Elevator {
  ...
  private static double liftLevels[] = {
    0,
    ROCKET_CARGO_HIGH,
    ROCKET_CARGO_MED,
    ROCKET_CARGO_LOW,
    ...
    CARGOSHIP_HATCH,
    FEEDER_STATION,
    LIFT_LEVEL_NEAR_FLOOR,
    LIFT_LEVEL_FLOOR
  };

  private static final int CLOSED_LOOP_VEL_SLOW = 600;
  ...
  public static void initialize() {
    ...
    masterMotor =
        configureMotor(
            HardwareIDs.LIFT_MASTER_TALON_ID,
            MASTER_REVERSE_MOTOR,
            ALIGNED_MASTER_SENSOR,
            kP,
            kI,
            kD,
            kF);
    slaveMotor =
        configureMotor(
            HardwareIDs.LIFT_SLAVE_TALON_ID, SLAVE_REVERSE_MOTOR, HardwareIDs.LIFT_MASTER_TALON_ID);

    initialized = true;
  }

  // closed-loop motor configuration
  private static TalonSRX configureMotor(
      int talonID,
      boolean revMotor,
      boolean alignSensor,
      double pCoeff,
  ...
  public static void runLift(double targetPosInches, int speedRpm, int accelRpm) {
    InputOutputComm.putDouble(
        InputOutputComm.LogTable.kMainLog, "Lift/CmdPosInches", targetPosInches);
    ...
    masterMotor.set(
        ControlMode.MotionMagic, targetPosInches / HardwareIDs.INCHES_PER_ENCODER_PULSE);
  }
